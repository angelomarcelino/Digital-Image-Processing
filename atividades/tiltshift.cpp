...
void multiplicaPonderacao() {
    for (int i = 0; i < img.rows; i++) {
        for (int j = 0; j < img.cols; j++) {
            saturatedImg.at<Vec3b>(i, j)[0] =
                (int)((float)saturatedImg.at<Vec3b>(i, j)[0] *
                      ((float)desfoque.at<uchar>(i, j) / 255.0));
            saturatedImg.at<Vec3b>(i, j)[1] =
                (int)((float)saturatedImg.at<Vec3b>(i, j)[1] *
                      ((float)desfoque.at<uchar>(i, j) / 255.0));
            saturatedImg.at<Vec3b>(i, j)[2] =
                (int)((float)saturatedImg.at<Vec3b>(i, j)[2] *
                      ((float)desfoque.at<uchar>(i, j) / 255.0));

            blurredImg.at<Vec3b>(i, j)[0] =
                (int)((float)blurredImg.at<Vec3b>(i, j)[0] *
                      ((float)desfoqueInv.at<uchar>(i, j) / 255.0));
            blurredImg.at<Vec3b>(i, j)[1] =
                (int)((float)blurredImg.at<Vec3b>(i, j)[1] *
                      ((float)desfoqueInv.at<uchar>(i, j) / 255.0));
            blurredImg.at<Vec3b>(i, j)[2] =
                (int)((float)blurredImg.at<Vec3b>(i, j)[2] *
                      ((float)desfoqueInv.at<uchar>(i, j) / 255.0));
        }
    }
}
...
void getPonderacao() {
    ...
    for (int i = 0; i < img.rows; i++) {
        for (int j = 0; j < img.cols; j++) {
            x = i;
            conta = (int)((0.5) * (tanh((x - l1) / d2) - tanh((x - l2) / d2)) *
                          255.0);
...
int main(int argc, char **argv) {
    ...
    createTrackbar(TrackbarName, "Tilt Shift", &alpha_slider, alpha_slider_max,
                   on_trackbar_alpha);

    sprintf(TrackbarName, "Foco: %d", focus_slider_max);
    createTrackbar(TrackbarName, "Tilt Shift", &focus_slider, focus_slider_max,
                   on_trackbar_focus);

    sprintf(TrackbarName, "Centro Focal: %d", position_slider_max);
    createTrackbar(TrackbarName, "Tilt Shift", &position_slider,
                   position_slider_max, on_trackbar_position);
